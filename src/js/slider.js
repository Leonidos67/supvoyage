...
document.addEventListener('DOMContentLoaded', function() {
        ...
        window.addEventListener('scroll', function() {
            ...
            // Запрашиваем новый кадр с текущей позицией скролла
            animationFrameId = requestAnimationFrame(function() {
                const scrollPos = window.scrollY || window.pageYOffset;
                handleScroll(scrollPos);
                animationFrameId = null;
            });
        }, { passive: true }); // Добавляем passive для оптимизации
        
        function handleScroll(scrollPos) {
            // Быстрое появление карточек при небольшом скролле
            if (scrollPos > 10) { // Уменьшил порог срабатывания
            ...
            if (scrollPos > 30) { // Уменьшил порог
                centerContainer.classList.add('scrolled');
            } else {
                centerContainer.classList.remove('scrolled');
            }
            
            // Параллакс эффект - линейный и быстрый
            const parallaxValue = Math.min(scrollPos * 0.3, 50); // Ограничиваем максимальное смещение
            centerContainer.style.transform = `translateY(${parallaxValue}px)`;
            centerContainer.style.transition = scrollPos > 30 ? 'none' : 'transform 0.2s ease-out'; // Отключаем анимацию при активном скролле
        }
